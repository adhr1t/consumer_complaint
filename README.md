...
# Model Performance
The Linear SVC model performed the best on test and validation sets. There's still heavy class imbalance, which naturally affected the model's efficacy to predict minority classes. However, a few majority classes make up an overwhelming majority of the Product classifications, and we want to maintain high accuracy in predicting those.
